...
if (grepl("\\*", raw_sumstats_file)) {  # Pattern was provided (multiple files, possibly per-chromosome)
  ss_files <- system(paste("ls", raw_sumstats_file), intern=T)  # Get vector of individual files from wildcard pattern
...
par(mar=c(5, 5, 4, 2))
make_qq(sumstats, "p_int", "QQ plot for interaction effect")
```

```{r interaction-loci}
if (min(sumstats$p_int, na.rm=T) > as.numeric(params$prune_threshold_int)) {
...
make_joint_plot(
  sumstats,
  main="Interaction joint plot"
)
```

```{r joint-loci}
if (min(sumstats$p_joint, na.rm=T) > as.numeric(params$prune_threshold_joint)) {
  ...
  if (nrow(joint_locus_tbl) == 0) {  # No joint loci? Print message.
    ...
    kable(joint_locus_tbl, booktabs=T,
          caption="Joint test loci")
  } else {  # >=50 joint loci? Print counts of joint/marginal overlap
    joint_locus_cnt_tbl <- data.frame(
      `Joint test locus count`=c(
        sum(joint_locus_tbl$`Genome-wide marginal locus`),
        sum(!joint_locus_tbl$`Genome-wide marginal locus`)
      )
    )
    rownames(joint_locus_cnt_tbl) <- c("In a marginal genetic effect locus",
                                       "Not in a marginal genetic effect locus")
    kable(joint_locus_cnt_tbl, booktabs=T,
